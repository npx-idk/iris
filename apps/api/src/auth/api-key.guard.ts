...
export class ApiKeyGuard implements CanActivate {
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const raw: string | undefined = req.headers['authorization']?.replace(/^Bearer\s+/i, '')
    if (!raw?.startsWith('iris_')) throw new UnauthorizedException('API key required')
    ...
    if (projectId && apiKey.projectId !== projectId) throw new ForbiddenException('API key does not belong to this project')
