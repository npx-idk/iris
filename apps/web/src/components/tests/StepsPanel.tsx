...
export function StepsPanel({
  ...
  const summary = { passed, failed, skipped: skipped + running, total, duration };

  return (
    <>
      <TooltipProvider delayDuration={400}>
        <div className="flex-1 overflow-y-auto p-4">
          <TestResults summary={summary} className="border-0 rounded-none bg-transparent">
            {total > 0 && (
              <>
                <TestResultsHeader className="border-b-0 px-0 pt-0 pb-2 mb-2">
                  <TestResultsSummary />
          ...
          {(() => {
            ...
            return prerequisites.map((prereq) => {
              ...
              let pPassed = 0, pFailed = 0, pSkipped = 0;
              ...
              const status = pFailed > 0 ? 'failed' : (pPassed === prereq.steps.length && prereq.steps.length > 0) ? 'passed' : 'skipped';

              return (
                <TestSuite key={prereq.id} name={prereq.name} status={status as any} className="mb-3 border-border bg-card overflow-hidden">
                  <TestSuiteName>
                    <span className="inline-flex items-center gap-3 min-w-0 flex-1">
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Link
                            ...
                            className="hover:underline inline-flex items-center truncate"
                            onClick={(e) => e.stopPropagation()}
                          >
                            <span className="truncate">{prereq.name || prereq.id}</span>
                          </Link>
                        </TooltipTrigger>
                      ...
                      <Badge variant="outline" className="text-[10px] px-1.5 py-0 h-5 bg-muted/50 font-normal uppercase tracking-wider text-muted-foreground shrink-0">
                        Prerequisite
                      </Badge>
                    </span>
                  </TestSuiteName>
                  <TestSuiteContent>
                    {prereq.steps.length === 0 ? (
                      <p className="text-xs text-muted-foreground px-4 py-3 border-t border-border">No steps</p>
                    ) : (
          ...
          {existingSteps.length > 0 && (() => {
            let mPassed = 0, mFailed = 0, mSkipped = 0;
            ...
            const status = mFailed > 0 ? 'failed' : (mPassed === existingSteps.length) ? 'passed' : 'skipped';

            return (
              <TestSuite name={test?.name || 'Main Steps'} status={status as any} defaultOpen className="mb-3 border-border bg-card overflow-hidden">
                <TestSuiteName>
                  {test?.name || 'Main Steps'}
                </TestSuiteName>
                <TestSuiteContent>
                  <DndContext
      ...
      <div className="border-t border-border p-4 shrink-0 bg-muted/10">
        <div className="relative rounded-xl border border-border bg-background shadow-sm focus-within:ring-1 focus-within:ring-primary overflow-hidden transition-all">
          <SlashCommandMenu
            value={instruction}
            ...
            onFocus={onFocus}
            disabled={isBusy}
            className="resize-none min-h-[60px] max-h-[120px] p-3 pb-10 text-sm bg-transparent border-none focus-visible:ring-0 focus-visible:ring-offset-0 placeholder:text-muted-foreground/70"
          />
