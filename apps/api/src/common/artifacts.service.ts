...
const PUBLIC_URL = env.minio.publicUrl
...
export class ArtifactsService implements OnModuleInit {
  ...
  async saveBrowserEvents(
    runId: string,
    events: { network: unknown[]; console: unknown[] }
  ): Promise<void> {
  ...
  async getBrowserEvents(
    runId: string
  ): Promise<{ network: unknown[]; console: unknown[] } | null> {
    try {
      ...
      this.logger.warn(`Could not load browser events for run ${runId}: ${err}`)
      return null
    }
  }
