...
Copy the example files and fill in the blanks (each file documents where to get every value):
...
At minimum you need to set `GEMINI_API_KEY` (from [Google AI Studio](https://aistudio.google.com/app/apikey)) and `BETTER_AUTH_SECRET` (`openssl rand -hex 32`).
...
Contributions are welcome — see [CONTRIBUTING.md](CONTRIBUTING.md) for local setup and PR guidelines. Security issues go through [SECURITY.md](SECURITY.md), not public issues.
