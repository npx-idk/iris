...
import { VariableCreateSheet } from "./VariableCreateSheet"
...
const COMMANDS = [
  {
    key: "variable",
    label: "Variable",
    description: "Insert a workspace variable",
  },
]
...
export function SlashCommandMenu({
  ...
  const items =
    menu?.phase === "commands" ? filteredCommands : filteredVariables
  const showCreate =
    menu?.phase === "variables" &&
    menu.query.trim().length > 0 &&
    !filteredVariables.find(
      (v) => v.name.toLowerCase() === menu.query.toLowerCase()
    )
  ...
  function openCreate() {
    if (!menu) return
    setCreateName(menu.query.trim())
    setCreateOpen(true)
  }

  function handleVariableCreated(name: string) {
    setPendingInsert(name)
    onVariableCreated()
  }
    ...
    <div className="relative">
      ...
      {menu && (
        ...
        >
          ...
          {/* Footer hint */}
          {totalItems > 0 && (
            <div className="flex gap-3 border-t border-border bg-muted/20 px-3 py-1.5">
              <span className="text-xs text-muted-foreground/50">
                ↑↓ navigate
              </span>
              <span className="text-xs text-muted-foreground/50">↵ select</span>
              <span className="text-xs text-muted-foreground/50">
                Esc {menu.phase === "variables" ? "back" : "close"}
              </span>
            </div>
          )}
        </div>
