...
export class WorkspaceService {
  ...
  async createVariable(userId: string, dto: CreateVariableDto) {
    ...
    const existing = await prisma.workspaceVariable.findUnique({
      where: { workspaceId_name: { workspaceId: workspace.id, name: dto.name } },
    });
    if (existing) throw new ConflictException(`Variable "${dto.name}" already exists`);

    const variable = await prisma.workspaceVariable.create({
      data: { workspaceId: workspace.id, name: dto.name, value: dto.value, isSecret: dto.isSecret ?? false },
    });
  ...
  async updateVariable(userId: string, varId: string, dto: UpdateVariableDto) {
    ...
    const variable = await prisma.workspaceVariable.findUnique({ where: { id: varId } });
    if (!variable || variable.workspaceId !== workspace.id) throw new NotFoundException('Variable not found');

    if (dto.name && dto.name !== variable.name) {
      const conflict = await prisma.workspaceVariable.findUnique({
        where: { workspaceId_name: { workspaceId: workspace.id, name: dto.name } },
      });
      if (conflict) throw new ConflictException(`Variable "${dto.name}" already exists`);
    }
  ...
  async getVariablesMap(workspaceId: string): Promise<Record<string, string>> {
    const vars = await prisma.workspaceVariable.findMany({ where: { workspaceId } });
