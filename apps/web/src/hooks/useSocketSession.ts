...
export function useSocketSession(sessionId: string | null, callbacks: SocketSessionCallbacks) {
  useEffect(() => {
    if (!sessionId) return
    const socket = io(
    ...
    socket.on('event', (e: any) => {
      ...
      if (e.type === 'author.step.completed') callbacks.onStepCompleted(e.pos, e.passed)
