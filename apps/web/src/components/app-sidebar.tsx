...
export function AppSidebar() {
  ...
  const testsHref = activeProject
    ? ROUTES.projectTests(activeProject.id)
    : ROUTES.dashboard
  const flowsHref = activeProject
    ? ROUTES.projectFlows(activeProject.id)
    : ROUTES.dashboard
  const dashboardHref = activeProject
    ? ROUTES.project(activeProject.id)
    : ROUTES.dashboard

  const isTestsActive =
    !!activeProject &&
    pathname.startsWith(ROUTES.projectTests(activeProject.id))
  const isFlowsActive =
    !!activeProject &&
    pathname.startsWith(ROUTES.projectFlows(activeProject.id))
  const isDashboardActive =
    !!activeProject && pathname === ROUTES.project(activeProject.id)
              ...
              <DropdownMenu>
                ...
                >
                  ...
                  {projects.map((project) => (
                    ...
                    >
                      <div className="flex size-5 shrink-0 items-center justify-center rounded bg-muted text-xs font-semibold">
            ...
            <SidebarGroupContent>
              <SidebarMenu>
                ...
                <SidebarMenuItem>
                  <SidebarMenuButton
                    asChild
                    isActive={pathname === ROUTES.usage}
                    tooltip="Usage"
                  >
        ...
        <SidebarFooter>
          <SidebarMenu>
            <SidebarMenuItem>
              <SidebarMenuButton
                size="lg"
                tooltip={session?.user.email ?? "Account"}
              >
                <div className="flex aspect-square size-8 shrink-0 items-center justify-center rounded-full bg-muted text-xs font-semibold text-muted-foreground">
                  {initial}
                </div>
